...
if (
  ...
  localStorage.getItem("ville") == undefined
) {
...
FirstPageForm1.addEventListener("input", function () {
  if (
    !isNaN(FirstPageForm1.value) &&
    FirstPageForm1.value != "" &&
    !FirstPageForm1.value.includes(" ") &&
    !FirstPageForm1.value.includes(" ")
  ) {
    FirstPageForm1.classList.replace("prePageInputLarge", "prePageInputSmol");
    FirstPageForm2.classList.replace(
      "prePageInputZipHide",
      "prePageInputZipShow"
    );
  } else {
...
cityPosForm1.addEventListener("input", function () {
  if (
    !isNaN(cityPosForm1.value) &&
    cityPosForm1.value != "" &&
    !cityPosForm1.value.includes(" ") &&
    !cityPosForm1.value.includes(" ")
  ) {
...
function searchWeather(ville) {
  ...
  fetch(
    `https://api.openweathermap.org/data/2.5/weather?${method}=${ville}${pays}&APPID=${key}&units=${units}`
  ) //A faire
    ...
    .then((result) => {
      ...
      if (result.cod == 404) {
        ...
        alert(
          "Erreur ! Le site est en maintence suite à un problème d'API, retour à l'écran de base."
        );
        resetVille();
        ...
        if (forecastOnce) {
          ...
          fetch(
            `https://api.openweathermap.org/data/2.5/forecast?q=${ville}&APPID=${key}&units=${units}`
          )
...
function playPluie() {
  let pluiePlay = false;
  if (
    (valeurMeteoTEMP == "Pluie" || valeurMeteoTEMP == "Orage") &&
...
function EclairSound() {
  if (musique.playing() == true) {
    ...
    musiqueEclair._src = `musiques/Eclair/${Math.floor(
      3 * Math.random() + 1
    )}.mp3`;
    musiqueEclair.load();
    musiqueEclair.play();
...
function changebg(valeurBG) {
  ...
  document.querySelectorAll(
    ".cityPos button"
  )[0].className = `lang cityPosButtonColor${valeurMeteoTEMP}${valeurBG}`;
  document.querySelectorAll(
    ".cityPos button"
  )[1].className = `lang cityPosButtonColor${valeurMeteoTEMP}${valeurBG}`;
  ...
  if (
    ...
    now.getMonth() == 0 ||
    now.getMonth() == 1 ||
    (now.getMonth() == 2 && now.getDate() <= 19)
  ) {
    BgInColor.style.backgroundImage = `url("medias/Plaineflat${tabSaisons[0]}.png")`;
  } else if (
    (now.getMonth() == 2 && now.getDate() >= 20) ||
    now.getMonth() == 3 ||
    now.getMonth() == 4 ||
    (now.getMonth() <= 5 && now.getDate() <= 19)
  ) {
    BgInColor.style.backgroundImage = `url("medias/Plaineflat${tabSaisons[1]}.png")`;
  } else if (
    (now.getMonth() >= 5 && now.getDate() >= 20) ||
    now.getMonth() == 6 ||
    now.getMonth() == 7 ||
    (now.getMonth() <= 8 && now.getDate() <= 21)
  ) {
...
function myTimer() {
  ...
  if (
    date.getHours() != HeureLancement &&
    tempverif == true &&
    SplashScreen == false
  ) {
    //Permet de savoir quand il y a un changement d'heure
    tempverif = false;
